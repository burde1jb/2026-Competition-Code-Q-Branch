...
public class FuelIntakeWristSubsystemJOE extends SubsystemBase {
    ...
    public double simposition = RobotConstants.FuelWristRetractgoal;//this starts the simulation at the min postion
    private double encoderGoal = RobotConstants.FuelWristExtendgoal;
    ...
    private double MinPostiion = RobotConstants.FuelWristRetractgoal; // rotations
    private double MaxPostition =  RobotConstants.FuelWristExtendgoal; // rotations
    ...
    public FuelIntakeWristSubsystemJOE() {
        FuelIntakeWristMotor = new SparkFlex(RobotConstants.FuelIntakeWristMotorCANidJOE, MotorType.kBrushless);
        FuelIntakeWristEncoder = FuelIntakeWristMotor.getAbsoluteEncoder();
        SparkMaxBuiltInPidController = FuelIntakeWristMotor.getClosedLoopController();
    ...
    public void goTo(double targetRotorPosition) {
      ...
      // Try to move until we're at the setpoint, then stop
        ...
        // })
        //     .until(()->MathUtil.isNear(targetRotorPosition, FuelIntakeWristEncoder.getPosition(), .01));
    }  
    ...
    public void setPosition(double rotations) {
        ...
        SparkMaxBuiltInPidController.setSetpoint(rotations, SparkBase.ControlType.kMAXMotionPositionControl, ClosedLoopSlot.kSlot0);
    }
    ...
    public void periodic() {
        //  SmartDashboard.putNumber("FuelIntake SIM Encoder", simposition);//why does this not include the offset?
        // SmartDashboard.putNumber("FuelInake Encoder", FuelIntakeWristEncoder.getPosition());//why does this not include the offset?
        // SmartDashboard.putNumber("motorPosition",FuelIntakeWristMotor.getEncoder().getPosition());
        // SmartDashboard.putNumber("FuelIntakeTemp",FuelIntakeWristMotor.getMotorTemperature());
        // SmartDashboard.putBoolean("FuelIntakeISAtSetpoint",SparkMaxBuiltInPidController.isAtSetpoint());
    ...
    private void SetupMotorConfig() {
        ...
        */
        motorConfig
            ...
            .d(DGain)
            .outputRange(-1, 1);
        motorConfig.closedLoop.maxMotion.allowedProfileError(.1);
        motorConfig.closedLoop.maxMotion.cruiseVelocity(10);
        motorConfig.closedLoop.maxMotion.maxAcceleration(10);
        ...
        motorConfig.absoluteEncoder.zeroOffset(0.72);
        motorConfig.absoluteEncoder.inverted(true);
        
        //ExternalEncoderConfig EEConfig = new ExternalEncoderConfig();
        SignalsConfig config = new SignalsConfig().isAtSetpointAlwaysOn(true);
...
//       FuelIntakeWristMotor.configureAsync(motorConfig,ResetMode.kResetSafeParameters, PersistMode.kPersistParameters);
//     }
//     if ((i != IGain)) {
//       IGain = i;
...
//       FuelIntakeWristMotor.configureAsync(motorConfig,ResetMode.kResetSafeParameters, PersistMode.kNoPersistParameters);
//     }
//     if ((A != MaxAccel)) {
//       MaxAccel = A;
