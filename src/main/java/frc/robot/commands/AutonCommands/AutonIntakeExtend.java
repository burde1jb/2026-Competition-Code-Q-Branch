...
public class AutonIntakeExtend extends Command {
    FuelIntakeSubsystem intakeSubsystem;
    boolean isItFinished;
    boolean intakeFinished;
    Timer timer;

    public AutonIntakeExtend(FuelIntakeSubsystem intakeSubsystem) {
        timer = new Timer();
    ...
    public boolean isFinished() {
        if (intakeSubsystem.wentTo(RobotConstants.FuelWristExtendgoal) || timer.get() >= 20.0) {
